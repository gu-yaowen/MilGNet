...
def fold_train_test_idx(folds, nFold, foldID):
    train_pos_idx = []
    train_neg_idx = []
    test_fold_idx = []
    for fold in range(nFold):
        if fold == foldID:
            continue
        train_pos_idx.append(folds['pos_' + str(fold)])
        train_neg_idx.append(folds['neg_' + str(fold)])
    train_pos_idx = np.concatenate(train_pos_idx, axis=1)
    train_neg_idx = np.concatenate(train_neg_idx, axis=1)
    train_neg_idx_ = np.random.choice(range(len(train_neg_idx[0])), size=len(train_pos_idx[0]), replace=False)
    train_neg_idx = [train_neg_idx[0][train_neg_idx_], train_neg_idx[1][train_neg_idx_]]
    train_fold_idx = np.concatenate([train_pos_idx, train_neg_idx], axis=1)

    test_fold_idx.append(folds['pos_' + str(foldID)])
    test_fold_idx.append(folds['neg_' + str(foldID)])
    test_fold_idx = np.concatenate(test_fold_idx, axis=1)
    idx = np.random.permutation(test_fold_idx.shape[1])
    idx_1, idx_2 = test_fold_idx[0][idx].flatten(), test_fold_idx[1][idx].flatten()
    test_fold_idx = np.vstack((idx_1, idx_2))
...
def load_data(args, fold, rp, neg_times=1, train_test='train'):
    ...
    if train_test == 'train':
        ...
        idx01 = read_pickle(prefix0 + '/0-1-0.idx.pkl')
        idx04 = read_pickle(prefix0 + '/0-1-1-0.idx.pkl')
    # adjlist00 = read_pickle(prefix0 + '/_0-0.adjlist.pkl')
    # adjlist01 = read_pickle(prefix0 + '/_0-1-0.adjlist.pkl')
    # # adjlist02 = read_pickle(prefix0 + '/0-2-0.adjlist.pkl')
    # # adjlist03 = read_pickle(prefix0 + '/0-3-0.adjlist.pkl')
    # adjlist04 = read_pickle(prefix0 + '/_0-1-1-0.adjlist.pkl')
    ...
    #
    # idx00 = read_pickle(prefix0 + '/_0-0.idx.pkl')
    # idx01 = read_pickle(prefix0 + '/_0-1-0.idx.pkl')
    # # idx02 = read_pickle(prefix0 + '/0-2-0.idx.pkl')
    # # idx03 = read_pickle(prefix0 + '/0-3-0.idx.pkl')
    # idx04 = read_pickle(prefix0 + '/_0-1-1-0.idx.pkl')
    ...
    if train_test == 'train':
        ...
        idx11 = read_pickle(prefix1 + '/1-0-1.idx.pkl')
        idx13 = read_pickle(prefix1 + '/1-0-0-1.idx.pkl')
    # adjlist10 = read_pickle(prefix1 + '/_1-1.adjlist.pkl')
    # adjlist11 = read_pickle(prefix1 + '/_1-0-1.adjlist.pkl')
    # # adjlist12 = read_pickle(prefix1 + '/1-2-1.adjlist.pkl')
    # adjlist13 = read_pickle(prefix1 + '/_1-0-0-1.adjlist.pkl')
    ...
    #
    # idx10 = read_pickle(prefix1 + '/_1-1.idx.pkl')
    # idx11 = read_pickle(prefix1 + '/_1-0-1.idx.pkl')
    # # idx12 = read_pickle(prefix1 + '/1-2-1.idx.pkl')
    # idx13 = read_pickle(prefix1 + '/_1-0-0-1.idx.pkl')
    ...
    return [[adjlist00, adjlist01, adjlist04],
            [adjlist10, adjlist11, adjlist13]], \
           [[idx00, idx01, idx04],
            [idx10, idx11, idx13]]
