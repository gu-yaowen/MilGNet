...
from utils.data import load_data, fold_train_test_idx, mydataset, collate_fc, get_features
...
def training(net, optimizer, train_loader, features_list, type_mask):
    ...
    for i, (train_g_lists, train_indices_lists, train_idx_batch_mapped_lists, y_train, batch_list) in enumerate(
        ...
        loss = loss_bce(output[:, 1], y_train.float()) # the same to above
...
def evaluate(net, test_loader, features_list, type_mask, y_true):
    ...
    with torch.no_grad():
        ...
        val_loss = loss_bce(val_pred[:, 1], y_true.float())
...
def run_model(args):
    ...
    folds = json.load(open(args.data_dir + f'fold_{args.rp}.json'))
    ...
    drug_protein = pd.read_csv(args.data_dir + '/drug_disease.csv', header=None).values
    ...
    for fold in range(args.nFold):
        ...
        if os.path.exists(args.save_dir + '/checkpoint/checkpoint_fold_{}_best.pt'.format(fold)):
            ...
            continue
        train_fold_idx, test_fold_idx = fold_train_test_idx(folds, args.nFold, fold)
        ...
        if args.only_test:
            ...
            for epoch in range(args.epoch):
                ...
                print('Epoch {:d} | Train loss {:.6f}'.format(epoch, train_loss))
                # validation
                if epoch == args.epoch - 1:
                    ...
                    f_csv.close()
                    best_weights = torch.load(args.save_dir + '/checkpoint/checkpoint_fold_{}.pt'.format(fold),
                                              map_location=args.device)
        ...
        json.dump(results, open(args.save_dir.format(rp) + f'/fold{fold}_pred_results.json', 'w'))
...
def parser():
    ...
    ap.add_argument('--dataset', default='B-dataset',
                    choices=['B-dataset', 'C-dataset', 'F-dataset', 'REDDA', 'Integret'])
    ...
    ap.add_argument('--rnn_type', default='max-pooling',
    ...
    ap.add_argument('--semantic_fusion', default='concatenation',
                    help='options: concatenation, attention, max-pooling, average.')
    ap.add_argument('--epoch', type=int, default=40, help='Number of epochs. Default is 100.')
    ...
    ap.add_argument('--num_ntype', default=2, type=int, help='Number of node types')
    ap.add_argument('--lr', default=0.0001)
    ap.add_argument('--weight_decay', default=1e-5)
    ap.add_argument('--dropout_rate', default=0.5, type=float)
    ...
    ap.add_argument('--data_dir', default='/home/gyw/MODDA_baseline/MHGNN-DTI-main/dataset/{}/')
...
if __name__ == '__main__':
    ...
    # ]
    etypes_lists = [
        [[None], [0, 1], [0, None, 1]],  # [0, 1, 0, 1], [2, 3, 2, 3], [4, 5, 4, 5], [0, 6, 7, 1], [2, 7, 6, 3]],
        [[None], [1, 0], [1, None, 0]]  # , [1, 0, 1, 0], [1, 2, 3, 0], [6, 3, 2, 7], [6, 7, 6, 7]]
    ]
