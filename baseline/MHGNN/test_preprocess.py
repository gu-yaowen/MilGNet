...
# def fold_test_idx(folds, foldID):
#     fold_idx = [[], []]
#     fold_posIdx = folds['pos_' + str(foldID)]
#     fold_negIdx = folds['neg_' + str(foldID)]
#     fold_idx[0] = fold_posIdx[0] + fold_negIdx[0]
#     fold_idx[1] = fold_posIdx[1] + fold_negIdx[1]
#     return fold_idx

def fold_test_idx(pos_folds, nFold, foldID):
    fold_idx = []
...
if __name__ == '__main__':
    data_set = ['B-dataset', 'C-dataset', 'F-dataset', 'REDDA', 'Integret']
    for dataset in data_set:
        ...
        #                 num_drug, num_protein, num_disease, num_se)
        # adjM = get_adjM(drug_drug, drug_protein, protein_protein,
        #                 num_drug, num_protein)
        ...
        for counter in range(num_repeats): # repeat ten times
            ...
            for foldID in range(nFold):
                ...
                test_fold_idx = fold_test_idx(folds, nFold, foldID)
                fold_drug_protein[test_fold_idx[0], test_fold_idx[1]] = drug_protein[
                    test_fold_idx[0], test_fold_idx[1]]

                adjM = get_adjM(drug_drug, fold_drug_protein, protein_protein,
                                num_drug, num_protein)
                drug_drug_list = {i: adjM[i, :num_drug].nonzero()[0] for i in range(num_drug)}
                drug_protein_list = {i: adjM[i, num_drug:num_drug + num_protein].nonzero()[0] for i in
                                     range(num_drug)}
                protein_drug_list = {i: adjM[num_drug + i, :num_drug].nonzero()[0] for i in range(num_protein)}
                protein_protein_list = {i: adjM[num_drug + i, num_drug:num_drug + num_protein].nonzero()[0] for
                                        i in
                                        range(num_protein)}
                ...
                for i, metapaths in enumerate(expected_metapaths):
                    for metapath in metapaths:
                        ...
                        if os.path.isfile(metapath_dir + '-'.join(map(str, metapath)) + '.npy'):
                            ...
                            edge_metapath_idx_array = get_metapath(metapath, num_drug, num_protein, metapath_dir)
