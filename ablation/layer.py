...
class MetaPathAggregator(nn.Module):
    ...
    def __init__(self, in_feats, hidden_feats, 
                 agg_type='sum', dropout=0.,
                 bn=False, mp='drdrdidi'):
        """
        Parameters
        ----------
        in_feats : int
    ...
    def forward(self, feature, mp_ins):
        # if mp_ins != 'drdidrdi':
        num_dr = self.mp.count('dr')
        num_di = self.mp.count('di')
        mp_ins_drug = mp_ins[:, :, :num_dr].long()
        mp_ins_dis = mp_ins[:, :, num_dr:].long()
        ...
        if self.agg_type in ['sum', 'mean']:
            ...
            mp_ins_feat = mp_ins_feat.reshape(mp_ins_feat.shape[0], mp_ins_feat.shape[1],
                                            mp_ins_feat.shape[2] * mp_ins_feat.shape[3])
            drug_agg = mp_ins_feat[:, :, :hd_feat]

            for i in range(num_dr - 1):
                drug_agg += mp_ins_feat[:, :, hd_feat * (i + 1):hd_feat * (i + 2)]
            drug_agg /= num_dr
            dis_agg = mp_ins_feat[:, :, hd_feat * num_dr:hd_feat * (num_dr + 1)]
            for i in range(num_di - 1):
                dis_agg += mp_ins_feat[:, :, hd_feat * (num_dr + i + 1):hd_feat * (num_dr + i + 2)]
            dis_agg /= num_di

            dis_feat = self.aggregator_drug_disease(drug_agg)
            for i in range(num_di - 1):
                dis_feat = (dis_feat + mp_ins_feat[:, :, hd_feat * (num_dr + i + 1):hd_feat * (num_dr + i + 2)]) /2
            
            drug_feat = self.aggregator_disease_drug(dis_agg)
            for i in range(num_dr - 1):
                drug_feat = (drug_feat + mp_ins_feat[:, :, hd_feat * (i + 1):hd_feat * (i + 2)]) / 2
            ...
            ins_emb = torch.cat((self.aggregator_drug(drug_feat),
                                    self.aggregator_dis(dis_feat)), dim=2)
        # else:
        # mp_ins_drug = mp_ins[:, :, [0, 2]].long()
        # mp_ins_dis = mp_ins[:, :, [1, 3]].long()
        # mp_ins_feat = torch.cat([feature['drug'][mp_ins_drug],
        #                         feature['disease'][mp_ins_dis]], dim=2)
        # hd_feat = mp_ins_feat.shape[3]
        # mp_ins_feat = mp_ins_feat.reshape(mp_ins_feat.shape[0], mp_ins_feat.shape[1],
